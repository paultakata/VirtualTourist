...
class PhotoCollectionViewController: UIViewController {
    ...
    var receivedPin: Pin! //Pin sent from MapViewController.
    ...
    func centreMapOnPin(pin: Pin) {
        
        //Centre the map around the user's pin at a moderate distance.
    ...
    func retryImageDownloadForPhoto(photo: Photo) {
        ...
        //Get image for photo object, save the context no matter what the result is.
        VirtualTouristClient.sharedInstance.getImageForPhoto(photo, completionHandler: {
            success, error in
    ...
    func getNewPhotoSet() {
        ...
        VirtualTouristClient.sharedInstance.getPhotosForPin(receivedPin, completionHandler: {
            success, error in
            
            if success {
                
                //Save the context and re-enable the newCollectionButton.
                dispatch_async(dispatch_get_main_queue(), {
                    CoreDataStackManager.sharedInstance.saveContext()
                ...
                //Give the user an alert and retry option, re-enable the newCollectionButton in case user doesn't retry.
                dispatch_async(dispatch_get_main_queue(), {
